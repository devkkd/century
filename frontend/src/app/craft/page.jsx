...
const stagesData = [
    {
        id: 1,
        stageNum: 'STAGE 1',
        title: 'Fibre Sourcing and Selection',
        titleItalicWord: 'and', // 'and' is italicized in the screenshot
        desc: [
    ...
    },
    {
        id: 3,
        stageNum: 'STAGE 3',
        title: 'Mould and Deckle Forming',
        desc: [
            ...
            <main className="min-h-screen pt-8 px-6 lg:px-10 max-w-[1400px] mx-auto">
                ...
                <div className="flex flex-col items-center text-center w-full max-w-5xl mx-auto mb-8">
                    ...
                    <h1 className="text-[#004342] text-3xl md:text-4xl lg:text-5xl font-playfair mb-8 tracking-tight leading-tight">
                        Five Stages, <span className="italic font-medium">No Shortcuts</span>, No Machines <br className="hidden md:block" /> That Matter
                    </h1>

                    {/* Intro Paragraph */}
                    <p className="text-[13px] md:text-sm text-gray-800 leading-relaxed max-w-4xl mx-auto">
                        This Is How A Sheet Of Century Papers Paper Comes Into Existence From Raw Cotton Waste To A Finished Sheet Ready To Hold Ink, Carry Seeds, Or Bind Into A Journal. Every Step Is Described Exactly As It Happens In Our Sanganer Workshop.
                    </p>
                </div>
                    ...
                    {stagesData.map((stage) => {
                        return (
                            <section key={stage.id} className="pt-10">
                                ...
                                {/* Changed to always use lg:flex-row so image is always on the right */}
                                <div className="flex flex-col gap-12 lg:gap-24 lg:flex-row">

                                    {/* Text Block */}
                                    <div className="flex flex-col flex-1 justify-center">
                                        <h2 className="text-[#004342] text-4xl md:text-5xl lg:text-5xl font-playfair tracking-tight mb-8">
                                            {/* Render italicized 'and' if applicable (like in Stage 1) */}
                                            {stage.titleItalicWord ? (
                                                <>
                                                    {stage.title.split(stage.titleItalicWord)[0]}
                                                    <span className="italic">{stage.titleItalicWord}</span>
                                                    {stage.title.split(stage.titleItalicWord)[1]}
                                                </>
                                            ) : (
                                                stage.title
                                            )}
                ...
                <section className="border-t border-gray-300 pt-10 mt-32">
                    ...
                    <h2 className="text-[#004342] text-4xl md:text-5xl lg:text-5xl font-playfair tracking-tight mb-16">
                        Beyond the Basic Sheet <span className="italic">Specialist</span> Processes
                    </h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-12 gap-y-12 lg:gap-y-16">
                        {specialistData.map((item) => (
                            <React.Fragment key={item.id}>
                                ...
                                {/* Image Block */}
                                <div className={`relative w-full aspect-[3/3.6] rounded-[32px] overflow-hidden ${item.imgOrder}`}>
                                    <Image
                                        src={item.image}
                                        alt={item.title}
                                        fill
                                ...
                                {/* Text Block */}
                                <div className={`flex flex-col justify-center ${item.txtOrder}`}>
